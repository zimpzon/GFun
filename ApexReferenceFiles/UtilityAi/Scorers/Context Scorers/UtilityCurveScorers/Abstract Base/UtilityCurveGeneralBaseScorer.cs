...
namespace Apex.Examples.AI
{
    ...
    public abstract class UtilityCurveGeneralBaseScorer : IContextualScorer
    {
        ...
        /// <exception cref="System.NotImplementedException">In case new utility curve types are added, they should also have two cases in this method implementation (one case for reversed and one for normal).</exception>
        protected float GetUtilityScore(float input)
        {
            ...
            switch (this.curveType)
            {
                ...
                {
                    return Utilities.GetLinear(input, this.curveFactor, this.curveMax) * this.scoreMultiplier;
                }

                case UtilityCurveType.ReversedLinear:
                {
                    return Utilities.GetReverseLinear(input, this.curveFactor, this.curveMax) * this.scoreMultiplier;
                }

                case UtilityCurveType.Exponential:
                {
                    return Utilities.GetExponential(input, this.curveFactor, this.curveMax) * this.scoreMultiplier;
                }

                case UtilityCurveType.ReversedExponential:
                {
                    return Utilities.GetReverseExponential(input, this.curveFactor, this.curveMax) * this.scoreMultiplier;
                }

                case UtilityCurveType.Logistic:
                {
                    return Utilities.GetLogistic(input, this.curveFactor, this.curveMax) * this.scoreMultiplier;
                }

                case UtilityCurveType.ReversedLogistic:
                {
                    return Utilities.GetReverseLogistic(input, this.curveFactor, this.curveMax) * this.scoreMultiplier;
                }

                case UtilityCurveType.Logit:
                {
                    return Utilities.GetLogit(input, this.curveFactor, this.curveMax) * this.scoreMultiplier;
                }

                case UtilityCurveType.ReversedLogit:
                {
                    return Utilities.GetReverseLogit(input, this.curveFactor, this.curveMax) * this.scoreMultiplier;
                }

                case UtilityCurveType.Gaussian:
                {
                    return Utilities.GetGaussian(input, 1f, this.curveFactor, this.curveMax) * this.scoreMultiplier;
                }

                case UtilityCurveType.ReversedGaussian:
                {
                    return Utilities.GetReversedGaussian(input, 1f, this.curveFactor, this.curveMax) * this.scoreMultiplier;
                }
